resource "aws_iam_role" "codedeploy_ecs" {
  name = "${local.project}-codedeploy-ecs-role"
...
resource "aws_codedeploy_app" "gateway" {
  name             = "${local.project}-gateway-app"
...
resource "aws_codedeploy_deployment_group" "gateway" {
  ...
  load_balancer_info {
    target_group_pair_info {
      ...
      target_group {
        name = aws_lb_target_group.gateway_tg.name
      }

      target_group {
        name = aws_lb_target_group.gateway_tg_green.name
      }
    }
  }
