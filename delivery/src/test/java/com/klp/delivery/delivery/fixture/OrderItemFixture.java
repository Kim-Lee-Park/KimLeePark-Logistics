...
public class OrderItemFixture {
    ...
    public static OrderItemCommand OrderItemCommand() {
        return new OrderItemCommand(ORDER_ITEM_ID_SECOND, DEFAULT_HUB_ID_UUID_FIRST, ORDER_PRODUCT_NAME, ORDER_QUANTITY);
    }

    public static List<OrderItemCommand> orderItemCommandsDefault() {
        return List.of(
            OrderItemCommand(ORDER_ITEM_ID_FIRST, DEFAULT_HUB_ID_UUID_FIRST, ORDER_PRODUCT_NAME, ORDER_QUANTITY),
            OrderItemCommand(ORDER_ITEM_ID_SECOND, DEFAULT_HUB_ID_UUID_FIRST, ORDER_PRODUCT_NAME, ORDER_QUANTITY));
    }

    // 파라미터 기반 아이템 생성
    public static OrderItemCommand OrderItemCommand(UUID orderItemId, UUID hubId, String productName, int quantity) {
        return new OrderItemCommand(orderItemId, hubId, productName, quantity);
    }
    ...
    public static OrderItem createOrderItem(UUID orderItemId, UUID hubId, String productName, int quantity) {
        return new OrderItem(orderItemId.toString(), hubId.toString(), productName, quantity);
    }

    public static List<OrderItem> createOrderItems() {
        List<OrderItem> items = new ArrayList<>();
        items.add(createOrderItem());
        items.add(createOrderItem(ORDER_ITEM_ID_SECOND, DEFAULT_HUB_ID_UUID_FIRST, ORDER_PRODUCT_NAME, ORDER_QUANTITY));
        items.add(createOrderItem(ORDER_ITEM_ID_THIRD, DEFAULT_HUB_ID_UUID_SECOND, ORDER_PRODUCT_NAME, ORDER_QUANTITY));
        return items;
    }
    ...
    public static List<OrderItem> createOrderItemListWithDeliveryId() {
        List<OrderItem> items = new ArrayList<>();
        items.add(
            createOrderItem(ORDER_ITEM_ID_FIRST, DEFAULT_HUB_ID_UUID_FIRST, ORDER_PRODUCT_NAME, ORDER_QUANTITY)
        );
        return items;
    }
